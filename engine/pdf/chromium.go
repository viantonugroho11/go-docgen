...
//
// Design notes:
//   - No chromedp.Navigate: new tabs start at about:blank; on pooled tabs
//     SetDocumentContent replaces the document over the existing top-level
//     frame — no explicit reset needed.
//   - No chromedp.WaitReady: page.SetDocumentContent installs the DOM
//     synchronously on the browser side. PrintToPDF then serialises the
//     current layout tree.
